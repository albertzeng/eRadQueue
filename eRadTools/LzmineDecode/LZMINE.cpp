...
/********************************************************************
	...
	purpose: allow MINE-code output
*********************************************************************/
...
//	m_nOutCur = 0;
	m_bMem = TRUE;
	Encode();
	m_B64.Encode(m_CodeBuff,&m_OutBuffer[0],m_OutBuffer.size());
	pOutBuffer = (BYTE *)&m_CodeBuff[0];	
	nOutLength = m_CodeBuff.length();	
}
...
void LZARI::UnCompress(const BYTE *pInBuffer,int nInLength,const BYTE *&pOutBuffer ,int &nOutLength)
{
	unsigned char * p;
	int len;
	m_B64.Decode(&p,len,(unsigned char*)pInBuffer,nInLength);	
	m_pInBuffer = p;
	m_nInLength = len;
	m_nInCur = 0;
	m_bMem = TRUE;
...
BOOL DecodeLZMINE(std::string* pInBuf,std::string & pOutbuf)
{
	LZARI lzmine;
	const BYTE *lpszOut = NULL;
	const BYTE *lpszIn	= (BYTE *)&(*pInBuf)[0];
	int iOutLength=0;
	lzmine.UnCompress(lpszIn,pInBuf->size(),lpszOut,iOutLength);
	if (lpszOut!=0 || iOutLength>0)
		pOutbuf	= (char *)lpszOut;
	else
		pOutbuf ="";
	lzmine.Release();
...
BOOL API_DecodeLZMINE(CString* pstrIn, CString& strOut)
{
	try
	{
		std::string tempstr1,tempstr2;
		tempstr1=*pstrIn;
		DecodeLZMINE(&tempstr1,tempstr2);
		strOut=tempstr2.c_str();	
	}
	catch (...)
	{
		return FALSE;
